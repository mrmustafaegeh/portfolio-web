    ...
    <div className="p-8">
      ...
      {(project.challenges || project.outcome) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {project.challenges && (
            <div className={`p-4 rounded-xl border ${darkMode ? "bg-red-500/5 border-red-500/20" : "bg-red-50/50 border-red-100"}`}>
              <h5 className={`text-xs font-bold uppercase tracking-wider mb-2 ${darkMode ? "text-red-400" : "text-red-600"}`}>Challenge</h5>
              <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}>{project.challenges}</p>
            </div>
          )}
          {project.outcome && (
            <div className={`p-4 rounded-xl border ${darkMode ? "bg-green-500/5 border-green-500/20" : "bg-green-50/50 border-green-100"}`}>
              <h5 className={`text-xs font-bold uppercase tracking-wider mb-2 ${darkMode ? "text-green-400" : "text-green-600"}`}>Outcome</h5>
              <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}>{project.outcome}</p>
            </div>
          )}
        </div>
      )}
